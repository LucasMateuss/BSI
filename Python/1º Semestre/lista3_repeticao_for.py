...
def media_anual(temperaturas):
    ...
    """
    acima_media, media, x  = [], sum(temperaturas) / len(temperaturas), 0
...
def maiores_13(idades, alturas):
    ...
    """
    media, x, lista = sum(alturas) / len(alturas), 0, []
    for i in alturas:
        if i < media and idades[x] > 13:
            lista.append(i)
        x += 1
    return lista
...
def altera_salarios(salarios):
    ...
    """
    nv_salario, sm, aumento = [], 724, 0
    for i in salarios:
        if i <= sm:
            aumento = round(i * 1.2, 2)
            nv_salario.append(aumento)
        elif i <= (sm * 2):
            aumento = round(i * 1.15, 2)
            nv_salario.append(aumento)
        elif i <= (sm * 5):
            aumento = round(i * 1.1, 2)
            nv_salario.append(aumento)
        else:
            aumento = round(i * 1.05, 2)
            nv_salario.append(aumento)
    return nv_salario
