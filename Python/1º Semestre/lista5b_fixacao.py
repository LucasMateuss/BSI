...
def rosquinhas(n):
    ...
    """
    return "muitas rosquinhas" if n >= 10 else f"{n} rosquinhas"
...
def nomes_pontas_e_iniciais_do_meio(n):
    ...
    n = n.upper().split()
    nome_abrev = []
    nome_abrev.append(n[0])
    for i in n[1:-1]:
        if i not in "DE DA DOS DO DAS":
            nome_abrev.append(i[0])
    nome_abrev.append(n[-1])
    return " ".join(nome_abrev)
