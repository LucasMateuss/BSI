...
def situacao_aluno(nota1, nota2, nota3, faltas, aulas_ministradas):
    ...
    presenca = (faltas * 100) / aulas_ministradas
    if presenca > 25:
        return "RF"
    if media < 7:
        return "EF"
...
def aumento_preco(preco):
    ...
    """
    if preco <= 280:
        return (preco, 20, preco * 0.2, round(preco * 1.2, 2))
    elif preco <= 700:
        return (preco, 15, preco * 0.15, round(preco * 1.15, 2))
    elif preco <= 1500:
        return (preco, 10, preco * 0.1, round(preco * 1.1, 2))
    else:
        return (preco, 5, preco * 0.05, round(preco * 1.05, 2))
...
def converte_hora_24_para_12(horario):
    ...
    if horas == "00":
        horas = horas.replace("00", "12")
        return(f"{horas}:{minutos} am")
    elif horas >= "01" and horas <= "11":
        return(f"{int(horas)}:{minutos} am")
    elif horas >= "12" and horas < "13":
        return(f"{int(horas)}:{minutos} pm")
    elif horas >= "13" and horas < "24":
        return(f"{int(horas) - 12}:{minutos} pm")
...
def converte_hora_12_para_24(horario):
    ...
    horas, minutos = horario.split(":") 
    if horas == "12" and minutos[2:] == "am":
        horas = horas.replace("12", "00")
        return(f"{horas}:{minutos[:2]}")
    elif horas > "0" and horas < "12" and len(horas) == 1 and minutos[2:] == "am":
        return(f"0{horas}:{minutos[:2]}")
    elif horas > "0" and horas < "12" and minutos[2:] == "am":
        return(f"{horas}:{minutos[:2]}")
    elif horas >= "12" and horas < "13" and minutos[2:] == "pm":
        return(f"{horas}:{minutos[:2]}")
    elif horas > "0" and horas < "12" and minutos[2:] == "pm":
        return(f"{int(horas) + 12}:{minutos[:2]}")
...
def conta_combustivel(qtde_litros, tipo_combustivel, tipo_pagamento):
    ...
    """
    precoA = qtde_litros * 3.159 
    precoG = qtde_litros * 3.739
    precoD = qtde_litros * 3.090
    if tipo_combustivel == "a":
        if tipo_pagamento == "v" or tipo_pagamento == "d":
            return round(precoA - (precoA * 0.1), 2)
        else:
            return round(precoA, 2) 
    elif tipo_combustivel == "g":
        if tipo_pagamento == "v" or tipo_pagamento == "d":
            return round(precoG - (precoG * 0.1), 2)
        else:
            return round(precoG, 2)
    elif tipo_combustivel == "d":
        if tipo_pagamento == "v" or tipo_pagamento == "d":
            return round(precoD - (precoD * 0.1), 2)
        else:
            return round(precoD, 2)
...
def comprar_frutas(morango=0, uva=0):
    ...
    """
    precos1 = morango * 2.5 + uva * 1.8
    precos2 = morango * 2.5 + uva * 1.5
    precos3 = morango * 2.2 + uva * 1.8
    precos4 = morango * 2.2 + uva * 1.5

    if (morango + uva) > 8:
        if morango <= 5 and uva <= 5:
            return round(precos1 - (precos1 * 0.1), 2)
        elif morango <= 5 and uva > 5:
            return round(precos2 - (precos2 * 0.1), 2)
        elif morango > 5 and uva <= 5:
            return round(precos3 - (precos3 * 0.1), 2)
        elif morango > 5 and uva > 5:
            return round(precos4 - (precos4 * 0.1), 2)
    elif (morango + uva) <= 8:
        if morango <= 5 and uva <= 5:
            if precos1 > 25:
                return round(precos1 - (precos1 * 0.1), 2)
            else:
                return round(precos1, 2)
        elif morango <= 5 and uva > 5:
            if precos2 > 25:
                return round(precos2 - (precos2 * 0.1), 2)
            else:
                return round(precos2, 2)
        elif morango > 5 and uva <= 5:
            if precos3 > 25:
                return round(precos3 - (precos3 * 0.1), 2)
            else:
                return round(precos3, 2)
        elif morango > 5 and uva > 5:
            if precos4 > 25:
                return round(precos4 - (precos4 * 0.1), 2)
            else: 
                return round(precos4, 2)
