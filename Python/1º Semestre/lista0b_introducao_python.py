...
def aumento_salarial(salario, porcentagem):
    ...
    return(round(salario + aumento, 2))
...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    """
    cigarros_fumados = cigarros_fumados_por_dia * (anos_fumando * 365)
    return(int((cigarros_fumados * 10) / 1440))
...
def salario(valor_hora, horas_mensais):
    ...
    desconto = (salario_bruto * 0.08) + (salario_bruto * 0.11) + (salario_bruto * 0.05)
    return(round(salario_bruto - desconto, 2))
...
def tinta(metros_pintar):
    ...
    """
    return(math.ceil(metros_pintar / 54))
